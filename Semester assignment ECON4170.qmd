...
ELP <- readr::read_csv("GUI_ENERGY_PRICES_202412312300-202512312300.csv") %>%
  clean_names() %>%
  mutate(
    Price_time = dmy_hms(mtu_cet_cest),
    hour_price = floor_date(mtu_cet_cest, "hour"), #setting the intervals to every hour
...
## other vaiables: water reservior levels and temprature history and load
...
reservior_levels <- readr::read_csv("GUI_WATER_RESERVOIRS_HYDRO_STORAGE_202412312300-202512312300.csv") %>%
  clean_names() %>%
  

head(reservior_levels)
