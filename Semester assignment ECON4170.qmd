...
struktur <- y_next ~ price_lag24 + price_lag168 + HDH_lag24 + demand_lag24 + supply_lag168 + dummy_hour + dummy_weekday + dummy_month
...
struktur_xreg <- y_next ~ HDH_lag24 + demand_lag24 + supply_lag168 + dummy_hour + dummy_weekday + dummy_month - 1
...
sarimax <- auto.arima(y, xreg = x, seasonal = TRUE , stepwise = TRUE, approximation = TRUE, max.p = 3, max.q = 3, max.P = 1, max.Q = 1, include.mean = FALSE) #to avoid duplicate constant

#summary from previous script just switching out model#
