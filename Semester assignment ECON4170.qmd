...
```{r}
# Don't need to run this as the correct lags are manually put in later. It takes about 5 min to run
...
```{r}
#This takes 3 hours to run but the data is saved and later used so no need to re RUN
...
```{r}
##This takes about 10 min runs the tables are saved in outpt folder
